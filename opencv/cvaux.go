...
//#cgo windows LDFLAGS: -lopencv_core242.dll -lopencv_imgproc242.dll -lopencv_photo242.dll -lopencv_highgui242.dll -lopencv_objdetect242.dll -lstdc++
import "C"
import (
	"unsafe"
)
...
func (this *HaarCascade) DetectObjects(image *IplImage) []*Rect {
	...
	for i := 0; i < (int)(seq.total); i++ {
		...
		// copy the rect in case its memory got released
		var r Rect
		r.Init(rect.X(), rect.Y(), rect.Width(), rect.Height())
